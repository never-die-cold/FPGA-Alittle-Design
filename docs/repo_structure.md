...
| 子目录 | 现状 | 要做什么 |
|:---|:---|:---|
| `src/riscv/` | ✅ 已开工 | **模块一核心**：自研 RISC-V 核 RTL。`plan.md` 剩余排期与学习路线（已完成部分归档见 `done/m1-first-phase-completed.md`）、`design_v0.md` 接口契约（唯一权威）、`pc/regfile/alu/decode/if_stage/core_top.v` 六个模块（v0 两级流水已仿真 PASS）。后续：Part B 三级+转发（9/25–9/28）、Part C 分支预测（9/29–10/1，2026-09-21 排期压缩）、M 扩展 `muldiv.v` 与最小 SoC 外壳收尾（原 `src/soc/` 已并入本目录） |
...
- 现状：板卡已到货并完成上板验证（全队共用 1 块，校园网 SSH 远程可用）；`setup.md` 从零复现指南与 `logs/` 记录模板已建（2026-09-20）；上板工程、运行脚本与实测记录待填充。
...
| 内容 | 要做什么 |
|:---|:---|
| `llm_log/` | **评委要看的"纠错轨迹"**：一条 = 一个设计决策点（提示词→首版→失败→纠错→结论→经验）。已有 23 条真实记录；铁律：别造假别后补，commit 是锚点，历史条目中的旧路径不改写 |
