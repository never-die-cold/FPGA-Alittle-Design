...
| 资源 | 说明 |
|:---|:---|
| [MrYanYe/NUECSDC_FPGA_National_Competition_Verilog_Skills_Test_V1](https://github.com/MrYanYe/NUECSDC_FPGA_National_Competition_Verilog_Skills_Test_V1) | ⭐ **国赛决赛 Verilog 上机能力测试真题**：2019/2020/2023/2024/2025 年题目 + 作者个人解答 + 离线资料；还有考场流程说明（考完监考验收波形）。对应我们 README 的"决赛限时上机考核"备赛 |
...
2. **实测与基线**：所有国奖作品都给出延迟/资源对比表。我们的"先测基线再谈优化"铁律与 `metrics/` 骨架正确；M3 实测时务必留原始日志。
3. **技术栈参考**：往届高分作品大量使用 HLS DATAFLOW + AXI DMA + PYNQ 可视化；我们的 CNN 协处理器的 DMA 搬运与 PS 侧采集脚本可借鉴其架构，但核心创新保留在自研核。
4. **决赛上机题**：立即把 MrYanYe 真题仓库纳入每周刷题材料（比 HDLBits 更贴真题）。
5. **风险提示**：自主选题赛道公开参考少、竞争看"完整作品 + 实测数据"；器件限制在 7 Series/Zynq-7000 内（PYNQ-Z2 合规）。
6. **工具版本**：官方推荐 2026.1，但允许 2025.2（我们已选 2025.2，理由：2026.1 BASIC 授权仿真受限，见 issue #2），需在报告中说明版本与可复现脚本。
