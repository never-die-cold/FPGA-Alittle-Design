...
| 板卡（PYNQ-Z2） | ✅ | 2026-09-20 到货并完成上板验证（联网 / Jupyter / base overlay / 点灯）；**全队共用 1 块**，上板时段群内报备 |
...
- [ ] 最小 SoC 外壳：指令 BRAM 预载 hex + LED / UART 二选一，仿真跑通 + 上板冒烟（落在 `src/riscv/`；板卡已到 2026-09-20，解除顺延，共用板需错峰）
...
- [ ] muldiv + 单指令测试
- [ ] SoC 外壳仿真 PASS + 上板冒烟（LED 闪 / 小程序，Part A 顺延项回补）
- [ ] design_v0.md 同步
