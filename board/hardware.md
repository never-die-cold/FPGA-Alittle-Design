...
> 依据：PYNQ 官方视频子系统文档（HDMI 前端分辨率限制）+ 2026-09-20 团队决议（视频源 = 笔记本 HDMI 输出，显示 = USB 采集卡回笔记本）。
> 决策记录：[`report/llm_log/2026-09-20-demo-hardware-chain.md`](../report/llm_log/2026-09-20-demo-hardware-chain.md)

## 1. 最终演示链路

```
笔记本 HDMI OUT ──HDMI线①──▶ [PYNQ-Z2 HDMI IN]
[PYNQ-Z2 HDMI OUT] ──HDMI线②──▶ [USB 采集卡] ──USB──▶ 笔记本（采集窗口看画面）
笔记本浏览器 ──▶ 校园网 ──▶ Jupyter（10.50.216.93:9090）控制与调参
PYNQ-Z2 PROG-UART ──Micro-USB──▶ 笔记本（串口日志 + 下载/调试）
```

- 笔记本一机两用：既是唯一视频源（HDMI 输出），又是显示终端（USB 采集卡回显）。实验室若有带 HDMI 的显示器，可替换采集卡方案。
...
| 🔴 必须 | HDMI 视频源 | 笔记本 HDMI 扩展输出测试图/视频；后期可换 HDMI 摄像头 |
...
| 🟢 可选 | 手机/相机 + 小三脚架 | 演示视频拍摄 |

## 3. 硬限制与参数口径

1. **分辨率锁定 720p**：PYNQ 官方文档明确 Z1/Z2 的 DVI 前端"因差分引脚速率限制，官方只支持到 720p"（视频流水线 142 MHz vs 1080p60 像素时钟 148.5 MHz）。源设备统一设 **1280×720@60**。
...
3. 通过后按 [`logs/README.md`](logs/README.md) 留记录（硬件连接列写明"笔记本 HDMI 源 + 采集卡显示 + 720p"），再进入 `src/vision/` 自研流水线开发。

## 5. 状态

- 接线方案：📌 已定（2026-09-20，笔记本源 + 采集卡显示）
- 采购：🚧 待执行（HDMI ×2 / 采集卡 / 12V；负责人：待认领）
- 验线：⬜ 未开始（线到后执行 §4）
