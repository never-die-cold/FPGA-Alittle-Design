...
- [ ] **CoreMark 移植启动（2026-09-21 新增，唯一正式基准，决策见 [docs/core_comparison.md](../../docs/core_comparison.md)）**：EEMBC coremark 拉取 + `core_portme.c/h` 裸机移植层（依赖 Part A 扩容与计数器）；linker script 按扩容后布局调整
...
### 4.4 M1 收口后：PicoRV32 对比实测（10/5 – 10/8，验证线）

> 2026-09-21 决策：排 M1 收口之后（不进 M1 关键路径），主责验证线。详见 [docs/core_comparison.md](../../docs/core_comparison.md)。
...
5. **演示层任务不进 M1 关键路径**：命题升级（参数化演示面板、软/硬推理切换、OSD 动效、检测-跟踪 stretch）的归属与排期见 `docs/proposal_upgrade.md`，全部排在 M2/M3 内嵌或其后的 stretch 窗口；本文 M1 排期零改动。
6. **分支纪律（2026-09-14 起）**：三线并行、每周日 PR 合并——分支名、目录边界与六条规矩见 §2；任何人（含组长）不直接 push `main`。
...
|:---|:---|
| 9/20 周日 | 首次周合并 |
| 9/27 周日 | Part A 验收 + 周合并 |
| 9/28 周一 | Part B 开工 |
| 10/1 国庆 D1 | Part B 验收 |
| 10/2–10/4 | Part C：BHT + benchmark + 对比数据 |
| 10/4 国庆 D4 | **M1 验收** |
| 11/4 18:00 | 作品提交截止 |
