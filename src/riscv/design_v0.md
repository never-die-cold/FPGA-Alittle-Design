...
`imem.v` 端口：`clk`（1 位输入）、`addr`（32 位输入）和 `rdata`（32 位寄存器输出）；
参数 `INIT_FILE` 指定 hex 路径。每个时钟上升沿执行
`rdata <= mem[addr[14:2]]`，因此地址在本拍给出、数据在下一拍可用。
...
`dmem.v` 端口：`clk`、`we`、`be[3:0]`、`addr[31:0]`、`wdata[31:0]` 为输入，
`rdata[31:0]` 为组合输出。读路径直接返回 `mem[addr[14:2]]`；写路径仅在时钟
上升沿且 `we=1` 时，按 `be` 分别更新四个字节车道。
