...
| `core_portme.c/h` | 本核移植层（D1/D2，待写） | 无 libc；`get_time()` 读计时计数器 |
| `core_main.c` | 官方 `core_main.c` 派生副本（D3，待写） | 仅加结果导出钩子；diff 入证据 |
| `link_coremark.ld` | 32KB 链接布局（C3，待写） | 排除观测块 `0x8000_7F00–0x7FFF` 与 `tohost` |
| 构建 | `Makefile` 增 `coremark` 目标（C3，待写） | 产出 `coremark.hex/.dis` |
